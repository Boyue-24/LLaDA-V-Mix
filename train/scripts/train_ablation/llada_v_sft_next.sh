...
BASE_RUN_NAME="llada_v_llava_next_ori"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node=${gpu_num} --nnodes=${num_node} --master_addr=${MASTER_ADDR} --master_port ${MASTER_PORT} --node_rank=${RANK} \
    ...
    --data_path "data/llava_next/llava_next_raw_format_processed.json" \
    --image_folder "data/llava_next/images" \
    --pretrain_mm_mlp_adapter="exp/llada_v_pre_ori/mm_projector.bin" \
